...
def data_source():
    ...
    with connect("database.db") as connection:
        ...
        connection.execute(
            "INSERT INTO DataModelTest (id, string, float_, int_, bool_, datetime_, date_, bytes_, list_, dict_, nested_data_model, nested_base_model, unique_, foreign_explicit) VALUES (2, 'STRING', 0.0, 0, 0, '2021-01-01 00:00:00', '2021-01-01', 'bytes', '[1, 2, 3]', '{\"key\": \"value\"}', 1, '{\"content\": \"CONTENT\"}', 'UNIQUE_2', 1)"
        )
        connection.commit()
...
def test_get_all_empty(data_source: SQLite3DataSource):
    assert data_source.get_all(DataModelTest, where={}) == [
        DataModelTest(
            id=1,
            string="STRING",
            float_=0.0,
            int_=0,
            bool_=False,
            datetime_=datetime(2021, 1, 1, 0, 0),
            date_=date(2021, 1, 1),
            bytes_=b"bytes",
            list_=[1, 2, 3],
            dict_={"key": "value"},
...
def test_get_simple(data_source: SQLite3DataSource):
    assert data_source.get_all(DataModelTest, where={"id": 1}) == [
        DataModelTest(
            id=1,
            string="STRING",
            float_=0.0,
...
def test_get_none(data_source: SQLite3DataSource):
    assert data_source.get_all(DataModelTest, where={"id": 3}) == []


def test_get_multiple_where(data_source: SQLite3DataSource):
    assert data_source.get_all(DataModelTest, where={"id": 1, "unique_": "UNIQUE"}) == [
        DataModelTest(
            id=1,
            string="STRING",
            float_=0.0,
